...
from memomemo.traces import TraceHarness, has_adapter
...
class LocomoOptimizer:
    ...
    def __init__(self, config: OptimizerConfig) -> None:
        ...
        self.trace_harness: TraceHarness | None = self._build_trace_harness()

    def _build_trace_harness(self) -> TraceHarness | None:
        backend = (self.config.trace_backend or "legacy").strip().lower()
        if backend == "legacy":
            return None
        if backend != "harness":
            raise ValueError(
                f"Unknown trace_backend {self.config.trace_backend!r}; "
                "expected 'legacy' or 'harness'."
            )
        benchmark = self.workspace_spec.benchmark
        if not has_adapter(benchmark):
            raise ValueError(
                f"trace_backend='harness' requested for benchmark "
                f"{benchmark!r} but no trace adapter is registered. "
                "Register one in memomemo.traces.adapters or revert to "
    ...
    @property
    def _harness_active(self) -> bool:
        return self.trace_harness is not None
    ...
    def run(self) -> dict[str, Any]:
        ...
        if candidates and not self.config.skip_scaffold_eval:
            ...
            write_post_eval_artifacts(
                ...
                skip_trace_slices=self._harness_active,
            )
            if self.trace_harness is not None:
        ...
        *,
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            prompt = build_progressive_proposer_prompt(
                ...
                trace_harness_dir=(
                    workspace_dir / "traces" if self._harness_active else None
                ),
            )
            if retry_note:
                prompt = f"{prompt}\n\n{retry_note}"
    ...
    def _copy_baseline_project_source_into(self, target_root: Path) -> list[str]:
        """Copy the baseline project source into ``target_root/src/memomemo``.
        ...
        return list(
            copy_benchmark_project_source(
                ...
                dest_pkg=target_root / "src" / "memomemo",
