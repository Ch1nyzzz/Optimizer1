...
def write_post_eval_artifacts(
    *,
    ...
    skip_trace_slices: bool = False,
) -> None:
    """Write compact eval summaries and (optionally) legacy trace slices.

    When `skip_trace_slices` is True, the per-candidate `trace_slices/`
    JSON files are not written. The TraceHarness backend takes over that
    responsibility under `traces/` instead. All other artifacts
    (eval_summary, candidate_score_table, retrieval_diagnostics, ...) are
    still written either way — they are used by paths beyond proposer
    trace inspection.
    """
    ...
    for candidate in candidates:
        ...
        if not skip_trace_slices:
            ...
            assert low_dir is not None and medium_dir is not None and high_dir is not None
            _write_json(
