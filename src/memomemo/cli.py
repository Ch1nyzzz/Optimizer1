...
from memomemo.claude_runner import DEFAULT_OPENCODE_MODEL
...
def main() -> int:
    ...
    optimize.add_argument(
        ...
        default="legacy",
        help=(
            "Trace recording backend. 'legacy' writes the existing "
            "trace_slices/ files. 'harness' writes a new structured "
            "traces/ tree (one JSONL per iteration/candidate, plus "
            "manifest). Default 'legacy' until the harness pipeline "
            "(M2-M6) lands; will flip to 'harness' once stable."
        ),
    )
    optimize.add_argument(
        "--trace-baseline",
        ...
        help=(
            ...
            "the diff baseline. Only meaningful when "
            "--trace-backend=harness. If omitted, the current run is "
            "treated as a baseline (no diff is computed)."
        ),
    )
