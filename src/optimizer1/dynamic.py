...
from optimizer1.scaffolds import build_scaffold
...
SOURCE_SCAFFOLD_CLASSES = {
    "memgpt_source": ("optimizer1.scaffolds.memgpt_scaffold", "MemGPTSourceScaffold"),
}
...
def load_candidate_scaffold(candidate: dict[str, Any], *, project_root: Path) -> MemoryScaffold:
    ...
    with context:
        if module_path:
            ...
            if candidate_root is not None and module_name.startswith("optimizer1.generated."):
                module_name = module_name.removeprefix("optimizer1.generated.")
...
def _source_project_src_root(path: Path) -> Path:
    ...
    for item in candidates:
        if (item / "optimizer1").is_dir():
            return item
    raise FileNotFoundError(
        f"source project path must contain optimizer1 package or project_source/src/optimizer1: {path}"
    )
...
def _isolated_memomemo_project(src_root: Path) -> Iterator[None]:
    """Temporarily import optimizer1 modules from a copied project source tree."""

    saved_modules = {
        name: module
        for name, module in list(sys.modules.items())
        if name == "optimizer1" or name.startswith("optimizer1.")
    }
    ...
    try:
        ...
        for name in [
            item for item in list(sys.modules) if item == "optimizer1" or item.startswith("optimizer1.")
        ]:
            sys.modules.pop(name, None)
