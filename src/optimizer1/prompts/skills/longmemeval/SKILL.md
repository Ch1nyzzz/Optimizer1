---
name: optimizer1-proposer-longmemeval
...
- **No task-specific knowledge in runtime behavior.** Do not hardcode answers,
  conversation/task ids, entity names, dates, gold strings, or judge quirks; do
...
- **Use traces and gold answers only to classify failure modes** — recurring
  evidence gaps, bad evidence ordering, retrieval misses, judging mismatches —
  as the input to a *general* fix, never as a lookup table.
...
You are evolving a memory layer that answers questions over long-term
conversation records. The runtime candidate is loaded through the source-backed
scaffold named in the iteration schema, typically `memgpt_source`. The usual
...
1. **Analyze.** Read the available evidence (see *Evidence interface* below) and
   ...
   context packing, answer synthesis, or judging mismatch. This is the most
...
2. **Hypothesize.** State one falsifiable hypothesis: a general mechanism that
   should improve held-out LongMemEval behavior, tied to a failure mode you
   classified.
...
- **does not use gold answers at inference time** and does not hardcode
  ...
  strings, or judge quirks, and does not branch on identifiers of saved tasks.
...
helpers, and utils. Do not modify the outer optimizer, evaluator, judge, raw
data loaders, or run artifacts as part of a candidate.
