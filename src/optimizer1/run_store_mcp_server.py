"""MCP tools backed by the unified ``runstore.db``.

The tools are grouped by the three evidence-store layers:

* ``runstore_artifact_*`` reads raw artifacts.
* ``runstore_fact_*`` reads structured trace/eval/modification facts.
* ``runstore_link_*`` reads explicit evidence graph links.
"""
...
@mcp.tool()
def runstore_fact_state(as_of_iteration: int | None = None) -> dict[str, Any]:
...
@mcp.tool()
def runstore_fact_candidate_outcome(
    iteration: int,
...
@mcp.tool()
def runstore_fact_compare_iterations(
    left: int,
    right: int,
...
@mcp.tool()
def runstore_fact_task_history(
    task_id: str,
...
@mcp.tool()
def runstore_fact_modification(
    iteration: int,
...
@mcp.tool()
def runstore_fact_proposer_call(iteration: int) -> dict[str, Any] | None:
...
@mcp.tool()
def runstore_fact_file_history(path: str, limit: int = 50) -> list[dict[str, Any]]:
...
@mcp.tool()
def runstore_link_explain_iteration(
    iteration: int,
    ...
    return {
        "iteration": int(iteration),
        "proposer_call": runstore_fact_proposer_call(iteration),
        "modification": runstore_fact_modification(
            iteration,
            include_diff=include_diff,
        ),
        "candidate_outcome": (
            runstore_fact_candidate_outcome(iteration, max_examples=5)
            if include_examples
            else None
        ),
...
@mcp.tool()
def runstore_link_explain_proposal(
    proposal_id: str | None = None,
    iteration: int | None = None,
    candidate_id: str | None = None,
    include_diff: bool = False,
    ...
    return {
        ...
        "proposer_call": runstore_fact_proposer_call(iter_value),
        "modification": runstore_fact_modification(iter_value, include_diff=include_diff),
        "candidate_outcome": (
            runstore_fact_candidate_outcome(
                iter_value,
...
def runstore_link_chain_task(
    ...
    history = runstore_fact_task_history(
        task_id,
