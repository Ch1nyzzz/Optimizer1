#!/usr/bin/env bash
# LoCoMo memory-benchmark comparison experiment —
...
#                  LoCoMo questions. It runs on YOUR OWN remote OpenAI-compatible
#                  endpoint — set EVAL_BASE_URL / EVAL_MODEL / EVAL_API_KEY in .env.
#
...
# Both arms share ONE primed seed frontier (prime_locomo_baseline), reused via
# --baseline-dir so the memgpt_source seed eval is not paid for twice.
#
# After the iterations each arm automatically evaluates its best train
# Pareto-frontier candidate on the held-out LoCoMo test split.
#
# The LoCoMo dataset is git-ignored, so a fresh clone has none. The script
# auto-runs `optimizer1.cli locomo prepare --allow-download` on first use; it
# fetches locomo10.json from the public snap-research mirror and writes the
...
SPLIT="${SPLIT:-train}"
LIMIT="${LIMIT:-0}"                 # 0 = the whole split (train = 80 LoCoMo questions)
...
if [ -z "$EVAL_BASE_URL" ] || [ -z "$EVAL_MODEL" ]; then
  echo "error: EVAL_BASE_URL / EVAL_MODEL not set. The LoCoMo scaffold queries a" >&2
  echo "       remote OpenAI-compatible endpoint — set both in .env." >&2
  exit 1
fi
...
mkdir -p logs runs
status_file="logs/launch_locomo_codex_azure_${TS}.status"
: > "$status_file"
printf '[%s] LAUNCHER start ts=%s iter=%s arms=%s split=%s limit=%s workers=%s\n' \
  "$(date -Is)" "$TS" "$ITERATIONS" "$ARMS" "$SPLIT" "$LIMIT" "$EVAL_WORKERS" \
  >> "$status_file"

contains() { case ",$1," in *",$2,"*) return 0;; *) return 1;; esac; }

# ---- dataset ------------------------------------------------------------
# data/ is git-ignored, so a fresh clone has no LoCoMo data. `prepare` downloads
# locomo10.json from the public snap-research mirror and writes the
# deterministic warmup/train/test splits.
ensure_locomo_dataset() {
  if [ -f data/locomo/locomo10.json ] && [ -f data/locomo/splits.json ]; then
    return 0
  fi
  printf '[%s] DATASET_PREPARE locomo (download)\n' "$(date -Is)" >> "$status_file"
  if ! python -m optimizer1.cli locomo prepare --allow-download >> "$status_file" 2>&1; then
    echo "error: 'locomo prepare' failed — see $status_file" >&2
    exit 1
  fi
  printf '[%s] DATASET_READY locomo\n' "$(date -Is)" >> "$status_file"
}

# ---- shared CLI fragment ------------------------------------------------
# Everything common to the baseline prime and both arms: LoCoMo dataset +
# remote eval model + Codex(Azure) proposer. The per-run bits (--run-id,
# --iterations, arm flags, test-frontier) are added by the callers.
common_args=(
  --locomo
  --split "$SPLIT"
  --limit "$LIMIT"
...
baseline_run_id="locomo_codex_azure_baseline_${SPLIT}_limit${LIMIT}_${TS}"
BASELINE_DIR="${BASELINE_DIR:-runs/${baseline_run_id}}"

prime_locomo_baseline() {
...
start_one() {
  ...
  if [ "$arm" = "default" ]; then
    ...
    run_id="locomo_codex_azure_default_${SPLIT}_${TS}"
  elif [ "$arm" = "organized" ]; then
    arm_args=(--organized --selection-policy default)
    run_id="locomo_codex_azure_organized_${SPLIT}_${TS}"
  else
  ...
  # on the held-out LoCoMo test split (--test-frontier-limit 0 = all test tasks).
