#!/usr/bin/env bash
# Mirror of launch_claudekimi_curai_three_tasks.sh with --trace-backend
...
driver_status="logs/claudekimi_curai_three_tasks_harness_${timestamp}.status"
mkdir -p logs runs
: > "$driver_status"
...
memory_common=(
  ...
  --selection-policy curai
  --trace-backend harness
)

locomo_run_id="locomo_memgpt_claudekimi_curai_harness_docker_iter30_train80_${timestamp}"
start_job "$locomo_run_id" "${memory_common[@]}" \
  --run-id "$locomo_run_id" \
  --locomo

longmemeval_run_id="longmemeval_memgpt_claudekimi_curai_harness_docker_iter30_train100_${timestamp}"
...
swebench_run_id="swebench_miniswe_deepseek_v4_flash_claudekimi_curai_harness_docker_iter20_trainfirst30_w10_t900_${timestamp}"
