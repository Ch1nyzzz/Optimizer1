#!/usr/bin/env bash
# Launch claudekimi proposer with --selection-policy curai on locomo,
...
driver_status="logs/claudekimi_curai_three_tasks_${timestamp}.status"
mkdir -p logs runs
: > "$driver_status"
...
locomo_run_id="locomo_memgpt_claudekimi_curai_docker_iter30_train80_${timestamp}"
start_job "$locomo_run_id" "${memory_common[@]}" \
  --run-id "$locomo_run_id" \
  --locomo

longmemeval_run_id="longmemeval_memgpt_claudekimi_curai_docker_iter30_train100_${timestamp}"
