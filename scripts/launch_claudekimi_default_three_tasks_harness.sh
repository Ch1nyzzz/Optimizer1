#!/usr/bin/env bash
# Mirror of launch_claudekimi_default_three_tasks.sh with --trace-backend
# harness added on every job. First-time run: no baseline supplied, so
# every trace is recorded as 'baseline' status. Subsequent runs can
# point --trace-baseline at one of these run dirs to enable diff mode.
set -u -o pipefail
...
scope="${SCOPE:-all}"  # all | memory | swebench
driver_status="logs/claudekimi_default_three_tasks_harness_${timestamp}.status"
mkdir -p logs runs
: > "$driver_status"
...
memory_common=(
  ...
  --trace-backend harness
)

locomo_run_id="locomo_memgpt_claudekimi_default_harness_docker_iter30_train80_${timestamp}"
start_job "$locomo_run_id" "${memory_common[@]}" \
  --run-id "$locomo_run_id" \
  --locomo

longmemeval_run_id="longmemeval_memgpt_claudekimi_default_harness_docker_iter30_train100_${timestamp}"
start_job "$longmemeval_run_id" "${memory_common[@]}" \
  --run-id "$longmemeval_run_id" \
  --longmemeval
fi  # end memory block

if [[ "$scope" == "all" || "$scope" == "swebench" ]]; then
...
swebench_run_id="swebench_miniswe_deepseek_v4_flash_claudekimi_default_harness_docker_iter20_trainfirst30_w10_t900_${timestamp}"
