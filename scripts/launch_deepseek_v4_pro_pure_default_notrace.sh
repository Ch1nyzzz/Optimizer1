#!/usr/bin/env bash
# Launch the pure default no-trace control runs across LoCoMo and
# LongMemEval-S. This is the baseline arm used to compare against
# organized default: default selection, clean patch base, no state.md,
# and no proposer-facing trace/RunStore MCP tools.
set -u -o pipefail
...
mkdir -p logs runs
status_file="logs/launch_deepseek_v4_pro_pure_default_notrace_${TS}.status"
: > "$status_file"
printf 'timestamp=%s\n' "$TS" >> "$status_file"
printf 'mode=pure_default_no_trace_harness\n' >> "$status_file"
...
start_one() {
  ...
  run_id="${task_label}_deepseek_v4_pro_pure_default_notrace_iter${ITERATIONS}_${TS}"
